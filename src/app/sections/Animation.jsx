...
const Animation = () => {
    ...
    const getMovies = async()=>{
        try{
            const moviesdata = await axios.get('https://moviesapi3.onrender.com/movies?genres=Animation')
                ...
                items.media.map((item)=>{
                    return (
                        <div className='flex gap-5' key={items.id}>
                            <SwiperSlide>
                                <AnimationCards moviename={`${items.moviename}`} movieposter={`${item.imageurl}`}/>
                            </SwiperSlide>
                        </div>
                    )
                })
